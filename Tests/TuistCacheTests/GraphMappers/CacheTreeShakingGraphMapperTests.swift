...
final class CacheTreeShakingGraphMapperTests: TuistUnitTestCase {
    ...
    func test_map_removes_pruned_targets_from_scheme() throws {
        ...
        // Then
        let gotProject = try XCTUnwrap(gotGraph.projects.first?.value)
        let gotScheme = try XCTUnwrap(gotProject.schemes.first)

        XCTAssertEqual(gotScheme.buildAction?.targets.count, 1)
        XCTAssertEqual(gotScheme.buildAction?.targets, [
            .init(projectPath: path, name: "second"),
        ])

        XCTAssertEqual(gotScheme.testAction?.targets.count, 1)
        XCTAssertEqual(gotScheme.testAction?.targets.map(\.target), [
            .init(projectPath: path, name: "second"),
        ])

        XCTAssertEqual(gotScheme.testAction?.codeCoverageTargets.count, 1)
        XCTAssertEqual(gotScheme.testAction?.codeCoverageTargets, [
            .init(projectPath: path, name: "second"),
        ])
    }
}
